...
slideshow:
  slide_type: ''
---
cfFR = cf.loc[:,'FR'].to_frame()
taFR = ta.loc[:,'FR10':'FRL0']
tpFR = tp.loc[:,'FR10':'FRL0']
```
...
data = cfFR
data['TA'] = taFR.mean(axis = 1)
data['TP'] = tpFR.mean(axis = 1)
...
Si on calccule xtrain.mean on retombe sur la valeur du biais, qui est aussi celle affichée à gauche en rouge. Le modèle a annulé tous les coeffficients du polynôme, on a une constante (égale au biais qui est égal à la valeur de xtrain.mean).
...
plot_tree(dt, feature_names=X.columns, filled=True, fontsize=10, max_depth=5, ax=ax)
plt.show()
```
...
cv = GridSearchCV(reg, param_grid=params)  # recherche par grille. CV pour cross validation.
...
=> Nécessité d'introduire un lag temporel entre les données des précipitations et les cf par exemple.
...
# Suite : travail personnele sur les données
...
Avec Grid search pour optimiser les hyperparamètres maintenant.
...
Deuxième susage de RFRavec gridsearch mais on fait de la K-cross-validation.
...
Refaire la même chose avec du LeaveOneGroupOut
...
Stochastic gardient descent sans grid search
...
Gradient boosting sans grid search
...
Gradient boosting avec grid search
...
Le but est de repartir des dataframes sur la france uniquement sans moyenner sur les régions pour pouvoir retravailler les données.
...
# Test avec le lag dans les précipitations

+++

On va chercher le meilleur lag dans les précipitations, en travaillant avec la méthode ridge.
...
class LagFeaturizerRegressor(BaseEstimator, RegressorMixin):
    def __init__(self, lag=1, base_model=None):
        self.lag = lag
        self.base_model = base_model

    def _make_lag_features(self, y):
        X, y_out = [], []
        for i in range(self.lag, len(y)):
            X.append(y[i-self.lag:i])
            y_out.append(y[i])
        return np.array(X), np.array(y_out)

    def fit(self, y, *_):
        X_lag, y_lag = self._make_lag_features(y)
        self.model_ = clone(self.base_model)
        self.model_.fit(X_lag, y_lag)
        return self

    def predict(self, y):
        X_lag, _ = self._make_lag_features(y)
        return self.model_.predict(X_lag)

from sklearn.model_selection import GridSearchCV, TimeSeriesSplit
from sklearn.linear_model import Ridge

model = LagFeaturizerRegressor(base_model=Ridge())

param_grid = {
    "lag": [1, 2, 3, 5, 7, 10],
    "base_model__alpha": [0.1, 1.0, 10.0]
}

tscv = TimeSeriesSplit(n_splits=5)

grid = GridSearchCV(model, param_grid, cv=tscv)
grid.fit(series)   # series = votre array 1D de valeurs temporelles
