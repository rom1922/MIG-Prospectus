...
for epoch in range(5): # on rappelle que les epochs correspondent au nombre de fois  qu'un algo d'apprentissage parcourt la totalité du jeu de donnée
    print(f"[Epoch {epoch}]")
    ...
    net2 = o1 @ w1
    #o2 = 1 / (1 + np.exp(-net2))  # sigmoid
    o2 = np.maximum(0,o2) #relu
...
# Data
# Les variables d'entrée
x = torch.tensor([[0.23, 0.82]])  # (1) équivalent des nd.array en numpy : tableaux n dimensionnels.

# Les poids de la couche d'entrée
w0 = torch.tensor([[0.1, 0.5],
                   [0.4, 0.3]], requires_grad=True)  # (2) paramètre requires_grad permet le calcul du gradient pour la rétropropagation
...
X_train = torch.from_numpy(X_train.astype("float32")) # float32 mieux car prend bcp moins de place que float64, mieux en pytorch
X_test  = torch.from_numpy(X_test.astype("float32"))
y_train = torch.from_numpy(y_train.astype("float32")).unsqueeze(dim=1) # le unsqueeze est là pcq pytorch préfère les vecteurs colonnes
y_test  = torch.from_numpy(y_test.astype("float32")).unsqueeze(dim=1)
```

```{code-cell} ipython3
# Mise en place dcu calcul de l'erreur
loss = nn.MSELoss()
...
class MyMLP(nn.Module):
    def __init__(self):
        ...
        self.layer0 = nn.Linear(in_features=10, out_features=64, bias=True)  # (1)
        self.layer1 = nn.Linear(in_features=64, out_features=5, bias=True)
        self.layer2 = nn.Linear(in_features=5, out_features=16, bias=True)
        self.layer3 = nn.Linear(in_features=16, out_features=1, bias=True)
