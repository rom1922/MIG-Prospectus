...
for epoch in range(7):
    ...
    # Architecture de notre réseau de neurones avec la fonction Relu
    net1 = x @ w0
    o1 = np.maximum(0,net1)  # sigmoid

    net2 = o1 @ w1
    o2 = np.maximum(0, net2)  # sigmoid
    ...
    delta1 = delta2 @ w1.T *(net1>0).astype(float)
    grad_w0 = x.T @ delta1
...
class MyMLP(nn.Module):
    def __init__(self):
        ...
        self.layer0 = nn.Linear(in_features=10, out_features=64, bias=True) 
        self.layer1 = nn.Linear(in_features=64, out_features=32, bias=True)  
        self.layer2 = nn.Linear(in_features=32, out_features=1, bias=True)
...
for i in range(len(X_test)) : 
    E_test  += (y_test[i] - model(X_test[i]))**2


E_test = E_test/len(X_test)
print(E_test)
```
