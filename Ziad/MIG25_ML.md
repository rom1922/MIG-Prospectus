...
# votre code ici
cf = pd.read_csv('data/CF_1d.csv', parse_dates = True , index_col = 'Date')
ta = pd.read_csv('data/TA_1d.csv' , parse_dates = True , index_col = 'Date' )
tp = pd.read_csv('data/TP_1d.csv', parse_dates = True  , index_col = 'Date')
```
...
cf_fr = cf.loc[:, 'FR']
cols = ta.columns[ta.columns.str.startswith("FR")]
ta = ta[cols]
tp = tp[cols]
```
...
ta_fr_mean = ta.mean(axis = 1)
tp_fr_mean = tp.mean(axis = 1)

data = pd.concat([ta_fr_mean, tp_fr_mean, cf_fr], axis = 1)
data.columns = ['TA', 'TP','CF']
data.head(5)
```
...
+++ {"id": "F2S_rzAlV46g", "jp-MarkdownHeadingCollapsed": true}
...
+++ {"id": "pMuH44LCBpS6", "jp-MarkdownHeadingCollapsed": true}
...
+++ {"id": "o8v9CrbxEp3Q", "jp-MarkdownHeadingCollapsed": true}
